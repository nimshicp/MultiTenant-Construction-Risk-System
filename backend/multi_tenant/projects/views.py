...
from .serializers import ProjectSerializer, TaskSerializer, TaskCommentSerializer, TaskChecklistItemSerializer
...
class ProjectDetailView(APIView):
    ...
    def get(self, request, project_id):
        ...
        if role in ('EMPLOYEE', 'VIEWER') and not project.team_members.filter(id=request.user.id).exists():
            return Response({'detail': 'Access denied.'}, status=status.HTTP_403_FORBIDDEN)
    ...
    def post(self, request, project_id):
        ...
        if assigned_to_id:
            if not project.team_members.filter(id=assigned_to_id).exists():
                return Response({'detail': 'Assigned user is not a member of this project team.'}, status=status.HTTP_400_BAD_REQUEST)
...
class TaskDeleteView(APIView):
    ...
    def delete(self, request, task_id):
        ...
        if not (is_pm or is_admin):
            return Response({'detail': 'Only managers or admins can delete tasks.'}, status=status.HTTP_403_FORBIDDEN)
...
class TaskCommentCreateView(APIView):
    ...
    def post(self, request, task_id):
        ...
        project = task.project
        if not (project.team_members.filter(id=request.user.id).exists() or 
                project.project_manager == request.user or 
                get_user_role(request.user) in ('ADMIN', 'SUPERADMIN')):
            return Response({'detail': 'Permission denied.'}, status=status.HTTP_403_FORBIDDEN)
